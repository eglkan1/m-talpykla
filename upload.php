...
if (isset($_POST["upload"])){
    ...
    if(in_array($fileActualExt, $allowed)){
            if($fileSize < 1000000){
                ...
                WHERE title='$title'") or die ($conn->error);
                $folderid = $folderid_res->fetch_array();
            */
                $uploads_dir = '../m-talpykla/images';
                move_uploaded_file($tname, $uploads_dir.'/'.$pname);
            
               $conn->query("INSERT INTO files(title,FILE,fk_FOLDERid) VALUES('$title','$pname',$id)") or die ($conn->error);
                

                $_SESSION['message'] = "Failas sėkmingai įkeltas!";
                $_SESSION['msg_type'] = "success";
                header("location: files.php?folder=$id");
